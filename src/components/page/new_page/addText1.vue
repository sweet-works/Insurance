...
            let left= e.clientX - contentBox.offsetLeft-e.offsetX-10+'PX'
        ...
        addParent(id, keyid, oper,leftOrRight) {
            ...
            for (let i = 0; i < datas.length; i++) {
                 if (datas[i].id == keyid) {
                      if(leftOrRight === 'left'){
                            let data_one_line_left = {id:keyid, type: 'text', content: oper, key: 'kuohao', value: oper}
                            newData.push(data_one_line_left, datas[i]); // 加前边
                      } else {
                           let data_one_line_right = {id:keyid, type: 'text', content: oper, key: 'kuohao', value: oper}
                           newData.push(datas[i],data_one_line_right); 
                      }
                 } else{
                     newData.push(datas[i]);
                 }
